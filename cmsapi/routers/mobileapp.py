...
from ..schemas import mobileapp as schema
...
def _retrieve_service(lang, offset, limit, uuid):
    ...
    if uuid == UUID('1c0a8927-bfb4-4215-a8fd-c41bba079d21'):  # handle files if location
        ...
        service_filter = MobileApp.parent_uuid == uuid

    with Session(engine) as session:
        statement = [select(MobileApp).
                     where(get_attr_by_lang(lang,
                                            de=MobileApp.active_de,
                                            en=MobileApp.active_en,
                                            fr=MobileApp.active_fr)).
                     where(service_filter).
                     where(MobileApp.meta_id != 'ZMSNote').
                     where(MobileApp.meta_id != 'codeblock').
                     order_by(get_attr_by_lang(lang,
                                               de=MobileApp.title_de,
                                               en=MobileApp.title_en,
                                               fr=MobileApp.title_fr)).
...
async def get_app_contact(
        ...
        offset: int = 0,
        limit: int = 10):
    return _retrieve_service(lang, offset, limit, UUID('58115e60-e80b-44c1-9a7d-bc65a42c9d5a'))


@router.get("/imprint", summary='Imprint', response_model=list[schema.Service])
async def get_app_imprint(
        lang: Lang = Lang.de,
        offset: int = 0,
        limit: int = 10):
    return _retrieve_service(lang, offset, limit, UUID('d15e37f5-b317-4b47-baf1-c203a34bd0ad'))


@router.get("/indexaz", summary='Index A-Z', response_model=list[schema.Service])
async def get_app_indexaz(
        lang: Lang = Lang.de,
        offset: int = 0,
        limit: int = 100):
    return _retrieve_service(lang, offset, limit, UUID('27bfdd98-c79b-4dd4-b69a-dd1bde958d0b'))


@router.get("/locations", summary='Locations', response_model=list[schema.Service])
async def get_app_locations(
        lang: Lang = Lang.de,
        offset: int = 0,
        limit: int = 10):
    return _retrieve_service(lang, offset, limit, UUID('1c0a8927-bfb4-4215-a8fd-c41bba079d21'))


@router.get("/privacypolicy", summary='Privacy Policy', response_model=list[schema.Service])
async def get_app_privacypolicy(
        lang: Lang = Lang.de,
        offset: int = 0,
        limit: int = 10):
    return _retrieve_service(lang, offset, limit, UUID('a7f29281-43e0-4607-871b-39e66204bb31'))


@router.get("/termsofservice", summary='Terms of Service', response_model=list[schema.Service])
async def get_app_termsofservice(
        lang: Lang = Lang.de,
        offset: int = 0,
        limit: int = 10):
    return _retrieve_service(lang, offset, limit, UUID('4b570198-0485-4f75-8242-9e5a7528351a'))
